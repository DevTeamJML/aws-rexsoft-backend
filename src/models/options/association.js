...
module.exports = (db) => {
  ...
  Role.belongsToMany(FormTemplate, {
    through: FormTemplatePermission,
    foreignKey: "role_id",
  });
  FormTemplate.belongsToMany(Role, {
    through: FormTemplatePermission,
    foreignKey: "form_template_id",
  });

  // only when agent row that has related to parent and needed to delete as well then only have to write relationship between them .
  FormTemplatePermission.belongsTo(FormTemplate, {
    foreignKey: "form_template_id",
    onDelete: "CASCADE",
  });
  FormTemplate.hasMany(FormTemplatePermission, {
    foreignKey: "form_template_id",
  });
  ...
  User.hasMany(Kpi, { foreignKey: "user_id" });
